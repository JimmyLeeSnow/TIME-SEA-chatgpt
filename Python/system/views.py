...
class UserStatusDisableView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            email_account = parameter_dic.get('emailAccount')
            user_info = json.loads(parameter_dic.get('userInfo', "{}"))
            open_id = user_info.get('openId')

            # 判断用户使用的登录方式
            account = email_account if email_account is not None else open_id
...
class ProhibitedTextDetectionView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            # 获取用户的信息
            parameter_dic = get_parameter_dic(request)
            email_account = parameter_dic['messages'].get('emailAccount')
            ...
            Users.objects.filter(detection_counts__gte=10).update(is_disable=1)
            ...
            try:
                ...
                if detection_value['Suggestion'] == 'Block':
                    # 严重违禁情况下次数+2
                    Users.objects.filter(filter_condition).update(detection_counts=F('detection_counts') + 2)
                    data = {
                    ...
                    # 普通违禁情况下次数+1
                    Users.objects.filter(filter_condition).update(detection_counts=F('detection_counts') + 1)
                    data = {
