  ...
  setup() {
    const data = ref([
      ...
      },
      {
        ...
        a: "标准对话(GPT-3) 每一次调用消耗1个Ai币 增强对话(GPT-4)每一次调用消耗 20个币,绘图类功能通用消耗5个币/次,当然也不乏免费功能 如超级实验室中的功能 包括其中的 智能对话 微软必应 克劳德 模型等 ,不过亦有缺陷 不是很稳定 偶尔可能回复失败 多尝试几次即可(NERVE支持GPT-3和GPT-4)"
      },
      {
        q: "绘图功能在哪里?",
      ...
      {
        q: "小程序数据和网页数据同步的吗?",
        a: "是的,数据是同步的 不过两者仅仅的交互为 Ai币互通 个人头像以及用户信息 对话收藏功能串联(小程序支持进入场景)"
      },
      {
        q: "小程序功能和网页端功能一样吗?",
      ...
      {
        q: "项目是什么开发的?部署难不难",
        a: "项目主要分为3个部分,Java(后端) UniApp(微信小程序) Vue(网页端) 实现, 中间件为 MySQL Redis,总体来说 很简单... 不懂群里可以问"
      }
    ])
...
// 鼠标松开事件处理
    ...
    function handleAdsorb() {
      ...
      // 不满足吸附条件则直接返回
      if (
          (ballLeft >= edgeThreshold && ballLeft <= windowWidth / 2 - edgeThreshold) ||
      ...
      // 动画过渡，替换250为需要的过渡时间
      ball.value.animate([{left: `${ballLeft}px`}, {left: `${newPositionX}px`}], 250);
    }
