  ...
  <div class="body" ref="scrollRef">
    ...
    <div v-else class="questions" style="margin: 20px 0;">
      <div v-for="(item, index) in conversationList" :key="index" class="item slide-animation">
        ...
        <div class="answer">
          <el-avatar class="flexShrink" :size="35" :icon="UserFilled" :src="require('../assets/blend.svg')"/>
      ...
      <div class="cache-content">
        <div class="cache-scrollbar">
          <el-scrollbar height="250px">
            <div class="cache-padding" v-for="(item,index) in blendCache.array" :key="index">
              <div class="cache-flex-space-between cache-margin">
                ...
                <div class="cache-selected">
                  <img
                      :src="blendCache.index===index?require('../assets/selected02.svg'):require('../assets/close.svg')"
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      //获取对话缓存数据
      let item = localStorage.getItem("blendCache");
      if (store.getters.userinfo) {
        if (!store.getters.userinfo) return loginVisible.value = true
        if (item) {
          blendCache.value = JSON.parse(item)
          let value = blendCache.value;
          ...
          localStorage.setItem("blendCache", JSON.stringify(blendCache.value))
        }
      }
    })
    ...
    function switchChat(index) {
      blendCache.value.index = index
      conversationList.value = blendCache.value.array[index].context
      localStorage.setItem("blendCache", JSON.stringify(blendCache.value))
    ...
    function clearDialogue(index) {
      if (index !== blendCache.value.index) {
        let i = parseInt(blendCache.value.index);
        if (index < i) {
          blendCache.value.index = (i - 1)
        }
        blendCache.value.array.splice(index, 1)
      }
      localStorage.setItem("blendCache", JSON.stringify(blendCache.value))
    }
    ...
    function writeDialogue() {
      ...
      let value = blendCache.value;
      blendCache.value.array[value.index].time = Date.now()
      if (item.length > 0) {
        blendCache.value.array[value.index].title = (item[item.length - 1].user).trim().slice(0, 25);
      }
      blendCache.value.array[value.index].context = item
      localStorage.setItem("blendCache", JSON.stringify(blendCache.value))
    }
    ...
    function createdNewChat() {
      blendCache.value.array.unshift({
        title: '新对话',
        time: Date.now(),
        context: []
      })
      blendCache.value.index = 0
