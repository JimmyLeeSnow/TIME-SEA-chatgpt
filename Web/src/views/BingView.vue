<template>
  <div class="body" ref="scrollRef">
    <div v-if="!conversationList.length" class="explain">
      <img class="logo" alt="Vue logo" src="../assets/bing.svg">
      <div class="expositoryCase">欢迎使用 NEW BING PLUS</div>
      <div class="consume">
        <el-icon>
          <Goods/>
        </el-icon>
    ...
    <div v-else class="questions" style="margin: 20px 0;">
      <div v-for="(item, index) in conversationList" :key="index" class="item slide-animation">
        ...
        <div class="answer">
          <el-avatar class="flexShrink" :size="35" :icon="UserFilled" :src="require('../assets/bing.svg')"/>
  ...
  <el-dialog v-model="dialogueDisplay" title="" width="430px" center style="background-color: rgb(27,30,32)">
    <div>
      <div class="cache-flex-center">
        <img alt="Vue logo" src="../assets/bing.svg" class="cache-img">
      </div>
      ...
      <div class="cache-content">
        <div class="cache-scrollbar">
          <el-scrollbar height="250px">
            <div class="cache-padding" v-for="(item,index) in bingCache.array" :key="index">
              <div class="cache-flex-space-between cache-margin">
                ...
                <div class="cache-selected">
                  <img
                      :src="bingCache.index===index?require('../assets/selected04.svg'):require('../assets/close.svg')"
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      //获取对话缓存数据
      let item = localStorage.getItem("bingCache");
      if (store.getters.userinfo) {
        if (!store.getters.userinfo) return loginVisible.value = true
        if (item) {
          bingCache.value = JSON.parse(item)
          let value = bingCache.value;
          ...
          localStorage.setItem("bingCache", JSON.stringify(bingCache.value))
        }
      }
    })
    ...
    function switchChat(index) {
      bingCache.value.index = index
      conversationList.value = bingCache.value.array[index].context
      localStorage.setItem("bingCache", JSON.stringify(bingCache.value))
    ...
    function clearDialogue(index) {
      if (index !== bingCache.value.index) {
        let i = parseInt(bingCache.value.index);
        if (index < i) {
          bingCache.value.index = (i - 1)
        }
        bingCache.value.array.splice(index, 1)
      }
      localStorage.setItem("bingCache", JSON.stringify(bingCache.value))
    }
    ...
    function writeDialogue() {
      ...
      let value = bingCache.value;
      bingCache.value.array[value.index].time = Date.now()
      if (item.length > 0) {
        bingCache.value.array[value.index].title = (item[item.length - 1].user).trim().slice(0, 25);
      }
      bingCache.value.array[value.index].context = item
      localStorage.setItem("bingCache", JSON.stringify(bingCache.value))
    }
    ...
    function createdNewChat() {
      bingCache.value.array.unshift({
        title: '新对话',
        time: Date.now(),
        context: []
      })
      bingCache.value.index = 0
    ...
    function webSocket({messages, index}) {
      if (typeof (WebSocket) == "undefined") {
        ...
        socket.value = new WebSocket(process.env.VUE_APP_WSS + "/bing/api/" + localStorage.getItem('token'));
        // TODO 建立连接
